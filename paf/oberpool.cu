...
class GPUpool {
	...
	vector<std::shared_ptr<udp::endpoint>> endpoints;
...
GPUpool::GPUpool(int ii) : d_fft_size(7 * 32 * 4 * 2), d_in_size(7 * 32 * 4 * 2), d_power_size(7 * 32 * 4), d_time_scrunch_size(27 * 7), d_freq_scrunch_size(27 * 7), nstreams(4) 
{
...
void GPUpool::execute(void)
{
    ...
    for (int ii = 0; ii < 6; ii++) {
        endpoints.push_back(std::make_shared<udp::endpoint>());
...
void GPUpool::receive_thread(void) {
    ...
    cout.flush();
    sockets[0].async_receive_from(boost::asio::buffer(rec_buffer), *endpoints[0], boost::bind(&GPUpool::receive_handler, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, *endpoints[0]));
}
